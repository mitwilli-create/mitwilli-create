...
For a decade I ran toward the hard live moments in network newsrooms — breaking-news control rooms at Al Jazeera English (*The Stream*), Fusion, AJ+, and HuffPost Live — where the system has to hold under deadline, judgment happens at speed, and there is zero tolerance for drift. I build AI the same way: agents that have to work on the first take, for an audience that would know immediately if they didn't.
...
At the Office of Engineering Strategy (OES), inside a large engineering organization, I shipped **two** production LLM agents into that senior-IC org — the comms-triage agent above and an executive "Voice DNA" RAG pipeline — and I run a private, forked-and-extended agentic pipeline (career-ops) that drives my own job search end to end.
